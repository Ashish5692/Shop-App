...
const Cart = () => {
  ...
  useEffect(() => {
    // Reduce function
    setTotalAmount(cart.reduce((acc, curr) => acc + curr.price, 0)); //reduce second value is initial value i.e 0
  }, [cart]); //value change when there is change/update/add/delete in cart array then total amount value will change
      ...
      {cart.length > 0 ? (
        <div className="max-w-[1200px] mx-auto flex flex-col md:flex-row justify-center">
          ...
          <div className="w-[100%] md:w-[60%] flex flex-col p-2">
            {cart.map((item, index) => {
              return <CartItem key={item.id} item={item} itemIndex={index} />; //item and index is passed as props to CartItem.jsx.. it will be destructure in CartItem.jsx
            })}
          </div>
